...
Embedding-based similarity search has become a cornerstone of modern job recommendation systems. Domain-specific models such as CareerBERT \cite{careerbert} achieve strong performance by leveraging structured taxonomies. However, general-purpose models such as sentence-transformers (e.g., all-MiniLM-L6-v2) have also shown excellent retrieval performance without task‑specific training, as demonstrated by Kurek et al. \cite{kurek2024}.
For tasks requiring higher precision in ranking, cross-encoder architectures are often preferred. As Singh notes, extensive evaluations show the superior performance of cross-encoders compared to traditional ranking methods \cite{cross-encoder-metric}. In line with these findings, this thesis employs a cross-encoder to jointly process job titles and extracted requirements, enabling more accurate and contextually aware similarity judgments.
This thesis therefore focuses on leveraging the high accuracy of a cross-encoder architecture for its core matching task,
by jointly processing the job title and its requirements so that the model can more effectively capture the relationships between them.

TODO: review the adjectives like scalable and limitations items, if they are all needed and repeating themselves. consider light wight as adjective.
...
datasets, making them unsuitable for deployment by smaller organizations
 or independent aggregators.
    \item Recent training methodologies often incorporate large‑scale
data augmentation strategies and hard‑negative mining, both of which
demand substantial computational resources and specialized model
development workflows.
\end{itemize}

To make these differences more explicit,
Table~\ref{tab:model_comparison} contrasts commercial job portals,
...
TODO: add explainability as a characteristic, light weight and interchangeable components
...
\begin{table}[h]
    ...
    \begin{tabular*}{\textwidth}{| l@{\extracolsep\fill} r || c | c | c |}
        ...
        \rotatebox{90}{Fine-Tuning Required} \\
...
In summary, although existing systems achieve strong performance in specific settings, there remains no approach that is both semantically robust and deployable using lightweight, fully open‑source components.
This thesis addresses this gap by proposing a job‑matching system that
employs small open‑source LLMs for automated requirements extraction
together with embedding‑based similarity search, thereby offering an
accessible, and computationally efficient alternative.
