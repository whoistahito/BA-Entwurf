...
Following the broader developments in job matching, this section examines research directly related to the two core techniques used in this thesis: (1) LRE and (2) TSS.
...
While fine-tuned models achieve strong performance, they require task-specific annotations and training. In contrast, methods such as few-shot prompting explored by Nguyen et al. demonstrate that general-purpose LLMs ``despite not being on par with traditional supervised models in terms of performance, can better handle syntactically complex skill mentions in skill
extraction tasks'' \cite{nguyen2024rethinkingskillextractionjob}. Motivated by these findings, this thesis explores methods to use small open-source LLMs to extract requirements.


\subsection{Embedding and Similarity Search in Job Matching}
\label{introduction:similaritysearch}
Embedding-based similarity search has become a cornerstone of modern job recommendation systems. Domain-specific models such as CareerBERT achieve strong performance by leveraging structured taxonomies \cite{careerbert}. However, a study showed that general-purpose models such as sentence-transformers have also shown excellent retrieval performance in job matching without task‑specific training \cite{kurek2024}. Kurek et al. note that ``the all-MiniLM-L6-v2 model with a chunk length of 768 exhibited outstanding performance, [...] establishing it as a highly effective tool for recruitment processes'' \cite{kurek2024}. Building on this foundation, this thesis employs the all-MiniLM-L6-v2 \cite{miniLm,MiniLMv2} model because of its strong performance in job matching
...
In summary, although existing systems achieve strong performance in specific settings, there remains no approach that is semantically robust, deployable using lightweight, fully open‑source components. This thesis addresses this gap by proposing a job matching system that employs LRE and TSS using small open‑source LLMs, thereby offering an accessible and computationally efficient alternative.
