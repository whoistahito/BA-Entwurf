...
\begin{itemize}
    \item a preprocessing step and LRE based on open-source LLMs, and
    \item a TSS module.
\end{itemize}

Although preprocessing the raw data is technically an essential step, it is treated as a part of the LRE pipeline in this work. Consequently, references to LRE hereafter implicitly encompass the necessary preprocessing of HTML content.

The remainder of this chapter details the architectural design and theoretical foundations of the proposed system. \Autoref{sec:data_structure} defines the unified data structure for requirements extraction and user profiles. \Autoref{sec:handling-raw-data} through \Autoref{sec:llm-evaluation} describe the requirements extraction pipeline, focusing on handling raw data, constraining LLM outputs via finite state machine (FSM), and evaluating extraction quality. Finally, \Autoref{sec:conception-vector-embedding} presents the strategy based on token-embedding, including the adaptation of the MaxSim algorithm for weighted field aggregation.
...
\begin{itemize}
    ...
    \item \textbf{Qualification} — formal education and certifications
\end{itemize}

These structured representations are encoded in a common Pydantic Model \cite{pydantic_validation_2025} for both job descriptions and user profiles. The requirements extraction module converts unstructured job descriptions into this schema so that they share the same structure as user profiles. With this common schema, the system can compare skills, experience, and qualification in a consistent way, forming the basis for the LRE step.
...
\section{Handling Raw Data} \label{sec:handling-raw-data}
The pipeline begins with raw HTML job postings retrieved by the YourJobFinder crawler. In their original form, these documents often contain large amounts of non‑informative content, such as JavaScript, CSS, advertisements, and unrelated boilerplate text. To create a structured and semantically meaningful input for the LLM, the HTML is converted into Markdown using the markdownify library \cite{python-markdownify}. This preprocessing step ensures that only the textual content relevant to the job description is retained, thereby reducing noise and improving extraction quality.
...
Therefore, a comparison of multiple models is required to determine which open‑source LLM is best suited for this task. \Autoref{tab:model_params} presents the LLMs that this thesis evaluates. These are lightweight, state-of-the-art open-source architectures available as of October 2025.
...
\begin{table}[h]
	...
	\begin{tabular}{|l|c|}
		...
		Qwen3 \cite{qwen3} & 8B  \\
		\hline
        GLM-4-9B-0414 \cite{glm2024chatglm} & 9B \\
		\hline
...
\section{Managing Input Data \& Chunking Strategies}
...
There are different methods of chunking text data for LLM processing and can be grouped into different categories. The following categories are used in this work:
\begin{itemize}
    ...
    \item \textbf{Structure-based chunking:} This method splits the text based on its structure and hierarchy \cite{pinecone}, in this case newlines, paragraphs, or Markdown headers. This method also aims to preserve the semantic integrity of the content \cite{pinecone}.
\end{itemize}
...
\section{LLM Output Evaluation} \label{sec:llm-evaluation}
...
\begin{itemize}
    ...
    \item \textbf{Alignment} — measures whether the model avoids including ``preferred'', ``bonus'', or nice-to-have requirements.
    \item \textbf{Readability} — examines whether the output adheres to the defined schema, is syntactically valid, and whether extracted requirements are correctly categorized into the appropriate fields (skills, experience, or qualifications).
...
representations using the all-MiniLM-L6-v2 model \cite{miniLm,MiniLMv2}. As discussed in \Autoref{introduction:similaritysearch}, this model offers strong zero‑shot performance for retrieval‑based tasks in job matching.\\
The skills fields are weighted more because, as studies show skills are more important in job application context. A study by Bone et al., which analyzed online job postings from 2018 to 2024, shows ``that individual skills, rather than formal education requirements, have become increasingly important features of job advertisements for AI roles'' \cite{skills-or-degree}.

The task of similarity search is to give a similarity score by embedding the extracted requirements and user profile, which have the same structure, and comparing them using the MaxSim approach.

\subsection{Vector Embedding} \label{sec:conception-vector-embedding}
An embedding effectively maps discrete objects, like words or documents, to points in a continuous vector space \cite{llm-from-scratch}. \Autoref{fig:embeddings-process} illustrates this transformation from raw input to a vector representation, which serves as an effective method to translate human language into computer language \cite{handson-llms-2024}.
...
\begin{figure}[h!]
    ...
    \caption{Embeddings of words that are similar will be close to each other in two-dimensional space (Source: \cite{handson-llms-2024}).}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{images/maxsim-matrix2}
...
\begin{itemize}
    ...
    \item Requirement \textit{Java}: Max match is 0.5 (from User: Python).
    \item Requirement \textit{Teamwork}: Max match is 0.6 (from User: Team player).
\end{itemize}
The final score for this section would be the average of these maximums: $(1.0 + 0.5 + 0.6) / 3 = 0.7$.
...
\subsection{Weighted Field Aggregation} \label{sub:conception-weighted-field}
