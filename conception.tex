...
\begin{figure}[h!]
    ...
    \caption{TODO:(add json array to each box and json to frame )Data structure mapping between job requirements and user profile for semantic matching.}
...
Figure~\ref{fig:fsm_masking} illustrates this process of sampling token for regular expression \texttt{[0-9]:[0-9]}, that can show for example a match score. Let's assume the Volcabulary $(\mathcal{V})$ contains only the set of Strings $\{\text{"2"}, \text{"14"}\,\text{"7"}\,\text{"T"}\,\textbackslash,\text{":"}\,\text{"="}\,\text{"i"}\}$. In the Beginning, the FSM is in state $q_0$ and only tokens valid, that represent a one-digit number, therefore $\text{"14"}$ and $\text{"T"}$ are masked and as the result their logits are set to zero. After sampling a valid digit, the FSM transitions to state $q_1$, where only the colon token is valid. Finally, after sampling the colon, the FSM reaches state $q_2$, where again only digit tokens are valid.
 At each generation step, the current FSM state determines which vocabulary tokens are valid continuations. Invalid tokens receive a mask value of zero, effectively removing them from consideration during sampling.
...
TODO: change this thesis to: why there is chunking needed and what approach is selected
