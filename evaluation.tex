...
However, despite these architectural guarantees of structural validity, the Readability scores do not reach the maximum value of 1.0. This gap highlights a critical distinction captured by the G-Eval metric, which evaluates not only whether the output is structured, but also ``whether extracted requirements are correctly categorized into the appropriate fields (skills, experience, or qualifications)'' (see \Autoref{metrics}).
