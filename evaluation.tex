...
\autoref{fig:overall_model_ranking} presents the aggregated overall model ranking derived from the evaluation metrics.
...
Qwen3‑8B achieves the highest overall performance, followed by GLM-4-9B-0414, Mistral-7B-Instruct-v0.3, and finally Llama-3.1-8B-Instruct.
Qwen3‑8B also yields the highest scores in structure, correctness, and Alignment as can be seen in \autoref{tab:model_performance}.
TODO: explain why the results for "Readability" look so high (or low) in the table.
Each value represents a mean score across the evaluation corpus, where 1.0 signifies perfect compliance with the metric.
\begin{table}[h]
    \centering
    ...
    \begin{tabular}{lccccc}
        \toprule
        \textbf{Model} & \textbf{Overall} & \textbf{Readability} & \textbf{Completeness} & \textbf{Correctness} & \textbf{Alignment} \\
        \midrule
        \textbf{Qwen3-8B} & \textbf{0.50} & \textbf{0.61} & 0.45 & \textbf{0.45} & \textbf{0.50} \\
        GLM-4-9B-0414           & 0.48 & 0.58 & \textbf{0.48} & 0.43 & 0.43 \\
        Mistral-7B-Instruct-v0.3         & 0.46 & 0.55 & 0.40 & 0.44 & 0.46 \\
        Llama‑3.1-8B-Instruct       & 0.41 & 0.49 & 0.37 & 0.38 & 0.39 \\
        \bottomrule
    \end{tabular}
    \vspace{0.2cm}
...
GLM-4-9B-0414 achieves the strongest completeness score, while Mistral-7B-Instruct-v0.3 offers more balanced but lower overall performance. Llama-3.1-8B-Instruct underperforms across all metrics, primarily due to hallucinated requirements and structural inconsistencies. \\
