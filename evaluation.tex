...
\section{Novel Approach vs Linkedin}
for answering RQ2 \autoref{itm:rq2} there has been an linkedin account created with following profile:
Skills:
Machine Learning
...
then the jobs that LinkedIn suggested were taken and compared with what job matching system of this approach. We assume user has ended their bachelor and were looking for a fulltime job. Linkedin has a feature that suggessts job based on the skills and experience and title in the profile like shown in \autoref{fig:linkedin_screenshot}
\begin{figure}[h!]
    \centering
...
first 23 jobs that linkedin suggested to the user were taken. First what this imaginary user would find suitable as job posting were marked then used job matching system in this approach to see if this system would rate the jobs same as how the user would prefer them. \autoref{fig:simlarity_score_distribtution} is the distribution in bucket of similrity rating using qwen3-8B.
