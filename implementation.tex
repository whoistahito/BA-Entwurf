...
Outlines library makes sure that LLM output matches the json schema \ref{lst:requirements_schema}. Although this is a good features but does not work always, as it can happen that the model does return an output that is not a valid json or missing a field, in that case it returns a Requirement Model where the fields have empty array.
