...
\begin{itemize}
	...
	\item \textbf{Outlines}: To enforce structured JSON / Pydantic Object generation from the LLMs, ensuring deterministic output formats.
...
\subsection{Output Structur Definition}
...
\begin{center}
	\begin{lstlisting}[language=json, caption={llm-defintion}, label={lst:llm-defintion},float=h]
{
  "qwen3-8b": {
...
Outlines library makes sure that LLM output matches the json schema \autoref{lst:requirements_schema}. Although this is a good features but does not work always, as it can happen that the model does return an output that is not a valid json or missing a field, in that case it returns a Requirement Model where the fields have empty array.
...
\begin{center}
    \begin{lstlisting}[language=python, caption={Implementation of MaxSim Logic}, label={code:compute_maxsim},float=h]
